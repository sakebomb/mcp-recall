...
- **The recorded project path is now always absolute.** `session-start` stores a `project_path` that `mcp-recall gc` uses to decide whether a project still exists, but it was taken verbatim from the hook payload's `cwd`, which is never validated — so a relative or empty value could be stored. `gc` reasons about that path's *parent*, which is only meaningful for an absolute path (`dirname("")` and `dirname("bare-name")` are both `"."`, which always exists), so such a database could be misread as an orphaned project. The path is now absolutised at its source, and a path that does not resolve to a real directory is not recorded at all rather than recorded as a guess. Note for anyone whose hook payload supplied a *relative* `cwd`: the project key is a hash of this value, so it changes — that project starts a new database and its earlier history becomes unreachable (nothing is deleted). Claude Code supplies an absolute `cwd`, so this is not expected in normal use (#213)
