...
**Deletion safety**: `gc --force` is the only thing in the project that deletes user data. Its deleted-project rule is "recorded `project_path` gone but its parent present ⇒ project deleted", which is valid *only* for an absolute path — `dirname("")` and `dirname("bare-name")` are both `"."`, which always exists. Non-absolute paths are therefore never deleted on that inference; they fall through to the staleness rule as `unrooted-fresh`/`unrooted-stale` (#214), reclaimable only once untouched past `--stale-days`. An absolute path whose parent is also gone (unmounted volume) stays `unverifiable` and is never deleted at any age. `session-start` only records a path that resolves to a real directory. When changing `src/gc/`, build a store containing the dangerous shapes and assert what survives; do not reason about it.
...
mcp-recall import <file>      # restore from a recall__export dump (--overwrite, --dry-run). --keep-project-key removed in #226 (rejected)
