...
| # | Title | Labels | Notes |
|---|-------|--------|-------|
| [#202](https://github.com/sakebomb/mcp-recall/issues/202) | Pin the signer identity when verifying the manifest | security, P1, S | **PR [#206](https://github.com/sakebomb/mcp-recall/pull/206) open.** GOTCHAS: (1) `--signer-workflow` is a **prefix-anchored** match with no terminator — `…/workflows/man` passes, and it stops short of `@refs/heads/<ref>`, so it does *not* pin the branch. Use `--cert-identity` (exact SAN) instead. (2) `--signer-workflow` also needs `<owner>/<repo>/<path>`; a bare path is rejected outright. |
